...
class TestIntegration:
    ...
    def test_environment_variable_substitution_integration(self, tmp_path: Path) -> None:
...
"""
        ...
        with responses.RequestsMock() as rsps:
            ...
            assert '{"repo": "awesome-checkout-action", "version": "Super Checkout V5"}' in synced_content

            # Verify multiple occurrences were all replaced
            lines = synced_content.split("\n")
            multiple_refs_section = [line for line in lines if line.startswith(("1. ", "2. ", "3. "))]
            assert "1. awesome-checkout-action" in multiple_refs_section[0]
            assert "2. awesome-checkout-action" in multiple_refs_section[1]
            assert "3. https://github.com/awesome-checkout-action" in multiple_refs_section[2]
    ...
    def test_real_github_integration(self, tmp_path: Path) -> None:
        ...
        envs_file = Path(__file__).parent.parent / ".github" / "repo-file-sync.envs.yaml"
    ...
    def test_dry_run_integration(self, tmp_path: Path) -> None:
        ...
        envs_file.write_text("- name: TEST_VAR\n  value: test_value\n", encoding="utf-8")
